...
async function submitIndexNow() {
  ...
  try {
    ...
    switch (status) {
      ...
      case 202:
        console.log(
          "✅ Accepted: URLs submitted successfully, pending key verification/processing.",
        );
        break;
      case 400:
      ...
      case 403:
        console.error(
          "❌ Forbidden: Invalid key (key not found, or file found but key does not match).",
        );
        break;
      case 422:
        console.error(
          "❌ Unprocessable Entity: URLs do not belong to the host, or key does not match the schema.",
        );
        break;
      case 429:
    ...
    console.error(
      "❌ Network or System Error occurred during submission:",
      error,
    );
    process.exit(1);
  }
