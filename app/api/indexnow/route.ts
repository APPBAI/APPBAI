...
export async function POST(request: Request) {
  try {
    ...
    if (urls.length === 0) {
      return NextResponse.json(
        { message: "No URLs found in sitemap.xml to submit" },
        { status: 200 },
      );
    }
    ...
    if (status === 200 || status === 202) {
      ...
      if (status === 403)
        errorDetail = "Key is invalid or verification file not found/matching";
      if (status === 422)
        errorDetail =
          "URLs do not belong to the host, or key does not match schema";
